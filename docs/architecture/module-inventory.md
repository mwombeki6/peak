...
| `usermanagement` | authorization, platform/tenant/property RBAC, invitation, lifecycle, `SessionContextPort`, `BreakGlassAccessPort`, `SupportPrivilegedAccessEvidencePort` and property staff-directory ports | Users, identities, RBAC, access matrix, sessions and ticket-bound break-glass grants | administration, identity and privileged-access events / `platform`, `internal`, `communication` | audit and reliability APIs; read-only `properties`, `tenant_modules` and `property_modules` session-bootstrap snapshot |
