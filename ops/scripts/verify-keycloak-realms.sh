...
EXPECTED_HOTEL_ORIGIN="${PEAK_APP_ORIGIN:-}"
EXPECTED_TENANT_ADMIN_ORIGIN="${PEAK_TENANT_ADMIN_APP_ORIGIN:-}"
...
for origin in \
  "$EXPECTED_HOTEL_ORIGIN" \
  "$EXPECTED_TENANT_ADMIN_ORIGIN" \
...
HOTEL_WEB_CLIENT="$(admin_get "$HOSPITALITY_REALM" "/clients?clientId=peak-web")"
TENANT_ADMIN_CLIENT="$(admin_get "$HOSPITALITY_REALM" "/clients?clientId=peak-tenant-admin")"
POS_CLIENT="$(admin_get "$HOSPITALITY_REALM" "/clients?clientId=peak-pos-desktop")"
PLATFORM_FORBIDDEN_HOTEL_CLIENT="$(admin_get "$PLATFORM_REALM" "/clients?clientId=peak-web")"
...
HOTEL_WEB_MAPPERS="$(client_mappers "$HOSPITALITY_REALM" peak-web)"
TENANT_ADMIN_MAPPERS="$(client_mappers "$HOSPITALITY_REALM" peak-tenant-admin)"
...
HOTEL_WEB_CLIENT="$HOTEL_WEB_CLIENT" \
TENANT_ADMIN_CLIENT="$TENANT_ADMIN_CLIENT" \
POS_CLIENT="$POS_CLIENT" \
PLATFORM_WEB_MAPPERS="$PLATFORM_WEB_MAPPERS" \
HOTEL_WEB_MAPPERS="$HOTEL_WEB_MAPPERS" \
TENANT_ADMIN_MAPPERS="$TENANT_ADMIN_MAPPERS" \
POS_MAPPERS="$POS_MAPPERS" \
PLATFORM_FORBIDDEN_HOTEL_CLIENT="$PLATFORM_FORBIDDEN_HOTEL_CLIENT" \
...
EXPECTED_HOTEL_ORIGIN="$EXPECTED_HOTEL_ORIGIN" \
EXPECTED_TENANT_ADMIN_ORIGIN="$EXPECTED_TENANT_ADMIN_ORIGIN" \
...
verify_spa("HOTEL_WEB_CLIENT", "peak-web", os.environ["EXPECTED_HOTEL_ORIGIN"], "HOTEL_WEB_MAPPERS")
verify_spa(
    "TENANT_ADMIN_CLIENT",
    "peak-tenant-admin",
    os.environ["EXPECTED_TENANT_ADMIN_ORIGIN"],
    "TENANT_ADMIN_MAPPERS",
)
verify_pos()

if parsed("PLATFORM_FORBIDDEN_HOTEL_CLIENT"):
    raise SystemExit("The platform realm must not contain the hotel operations client")
