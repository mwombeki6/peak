...
for name in \
  ...
  MINIO_ROOT_USER \
  PEAK_VERIFICATION_STORAGE_ENDPOINT \
  PEAK_VERIFICATION_STORAGE_ACCESS_KEY \
  PEAK_VERIFICATION_STORAGE_BUCKET \
  PEAK_VERIFICATION_STORAGE_REGION \
  PEAK_VERIFICATION_MALWARE_SCAN_HOST \
  PEAK_VERIFICATION_MALWARE_SCAN_PORT
do
  require_var "$name"
  reject_placeholder "$name"
done
...
for name in \
  ...
  MINIO_ROOT_PASSWORD \
  PEAK_VERIFICATION_STORAGE_SECRET_KEY \
  VALKEY_PASSWORD
do
  require_secret "$name"
done
...
for name in \
  ...
  PEAK_VERIFICATION_STORAGE_ENABLED \
  PEAK_VERIFICATION_MALWARE_SCAN_ENABLED \
  PEAK_VALKEY_ENABLED
do
  require_boolean "$name"
done
...
if [ "$(value_of PEAK_VERIFICATION_STORAGE_ENABLED)" != "true" ]; then
  fail "PEAK_VERIFICATION_STORAGE_ENABLED must be true in production"
fi

if [ "$(value_of PEAK_VERIFICATION_MALWARE_SCAN_ENABLED)" != "true" ]; then
  fail "PEAK_VERIFICATION_MALWARE_SCAN_ENABLED must be true in production"
fi
