...
OVERLAY_FILE="${OVERLAY_FILE:-$ROOT_DIR/ops/testing/compose.phase2-acceptance.yaml}"
PROJECT="${COMPOSE_PROJECT_NAME:-peak-phase2-acceptance}"
...
RESET="${PHASE2_RESET:-false}"
BUILD_IMAGE="${PHASE2_BUILD_IMAGE:-false}"
EVIDENCE_FILE="${PHASE2_EVIDENCE_FILE:-$ROOT_DIR/build/phase2-acceptance-evidence.json}"
...
api() {
  ...
  local args=(
    ...
    -H "X-Correlation-Id: phase2-${idempotency_key:-read}"
  )
...
root_password="${PHASE2_ROOT_PASSWORD:-$(random_password)}"
tenant_password="${PHASE2_TENANT_PASSWORD:-$(random_password)}"
other_password="${PHASE2_OTHER_PASSWORD:-$(random_password)}"
...
"${compose[@]}" --profile bootstrap run --rm --no-deps \
  ...
  -e PEAK_PLATFORM_BOOTSTRAP_FULL_NAME="Phase 2 Platform Root" \
...
tenant_payload="$(
  jq -nc '{
    name: "Phase 2 Acceptance Hotel Group",
    slug: "phase2-acceptance-group",
    planId: "20202020-0000-0000-0000-000000000001",
    legalName: "Phase 2 Acceptance Hotel Group Limited",
    tradingName: "Phase 2 Acceptance",
...
api POST "/api/v1/platform/tenants/$tenant_id/administrators" "$platform_token" 201 \
  "tenant-admin-provision" "$(
    jq -nc \
      ...
      '{
        fullName: "Phase 2 Tenant Administrator",
...
api POST "/api/v1/communication/contacts" "$tenant_token" 200 \
  "contact-create" "$(
    jq -nc --arg email "$contact_email" \
      '{fullName:"Phase 2 Managing Director",jobTitle:"Managing Director",email:$email}'
  )"
...
other_payload="$(
  jq -nc '{
    name:"Phase 2 Other Tenant",
    slug:"phase2-other-tenant",
    planId:"20202020-0000-0000-0000-000000000001",
    legalName:"Phase 2 Other Tenant Limited",
    entityType:"limited_company",
