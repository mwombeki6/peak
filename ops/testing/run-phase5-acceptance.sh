...
PHASE4_EVIDENCE="$EVIDENCE_DIR/phase4-acceptance.json"
NEWMAN_REPORT="$EVIDENCE_DIR/newman-private.json"
EVIDENCE_FILE="$EVIDENCE_DIR/phase5-acceptance.json"
...
tenant_password="${PHASE5_TENANT_PASSWORD:-P5-$(openssl rand -hex 18)}"
export PHASE3_TENANT_PASSWORD="$tenant_password"
export PHASE4_TENANT_PASSWORD="$tenant_password"
export PHASE3_EVIDENCE_DIR="$EVIDENCE_DIR"
export PHASE4_EVIDENCE_DIR="$EVIDENCE_DIR"
export COMPOSE_PROJECT_NAME="$PROJECT"
export OVERLAY_FILE="$ROOT_DIR/ops/testing/compose.phase5-acceptance.yaml"

if [[ "${PHASE5_REUSE_FOUNDATION:-false}" != "true" ]]; then
  "$ROOT_DIR/ops/testing/run-phase4-acceptance.sh"
fi
...
property_id="$(jq -er '.propertyId' "$PHASE4_EVIDENCE")"
contact_id="$(jq -er '.contactId' "$PHASE2_EVIDENCE")"
channel_id="$(jq -er '.channelId' "$PHASE2_EVIDENCE")"
pos_session_id="$(jq -er '.posSessionId' "$PHASE4_EVIDENCE")"
pos_order_id="$(jq -er '.posOrderId' "$PHASE4_EVIDENCE")"
...
collection="$ROOT_DIR/ops/testing/Peak-Phase-5.postman_collection.json"
podman run --rm \
  --network host \
  -v "$collection:/etc/newman/phase5.json:ro,Z" \
  -v "$EVIDENCE_DIR:/etc/newman/evidence:Z" \
  docker.io/postman/newman:alpine \
  run /etc/newman/phase5.json \
  --reporters cli,json \
