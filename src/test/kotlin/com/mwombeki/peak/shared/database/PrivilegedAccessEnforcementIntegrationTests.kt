...
class PrivilegedAccessEnforcementIntegrationTests @Autowired constructor(
    ...
    /**
     * The usage ledger carries a tenant_id, so it is protected by row-level
     * security rather than by table grants alone. What that buys is a default:
     * a role that is handed a grant but never given a policy reads nothing.
     *
     * Both directions are asserted together because the failure modes are
     * opposite and a policy change can easily cause either. Dropping the
     * policies would leave pms_platform blind, and dropping the ENABLE would
     * let the ungoverned role read every tenant's access history.
     */
    @Test
    fun `the usage ledger is readable by policy rather than by grant`() {
        val world = world(maxUses = 2)
        consume(world)
        assertTrue(
            countUsage(world.accessId, "consumed") > 0,
            "fixture should have written usage rows to read",
        )

        val ungoverned = "peak_rls_probe_${UUID.randomUUID().toString().replace("-", "").take(12)}"
        jdbcTemplate.execute("CREATE ROLE $ungoverned NOLOGIN NOBYPASSRLS")
        try {
            jdbcTemplate.execute(
                "GRANT SELECT ON TABLE platform_privileged_access_usage TO $ungoverned",
            )

            assertEquals(
                0,
                usageRowsVisibleTo(ungoverned),
                "a role with a grant but no policy must read nothing",
            )
            assertTrue(
                usageRowsVisibleTo("pms_platform") > 0,
                "platform operators must still read the ledger they are meant to review",
            )
        } finally {
            jdbcTemplate.execute(
                "REVOKE ALL ON TABLE platform_privileged_access_usage FROM $ungoverned",
            )
            jdbcTemplate.execute("DROP ROLE IF EXISTS $ungoverned")
        }
    }
    ...
    /**
     * Reads the ledger as [role] rather than as the test's own login, which is
     * the migration role and bypasses row-level security entirely. The role
     * name is generated by the caller, never supplied externally, which is
     * necessary because an identifier cannot be a bind parameter.
     */
    private fun usageRowsVisibleTo(role: String): Int =
        requireNotNull(
            jdbcTemplate.execute(ConnectionCallback { connection ->
                connection.createStatement().use { it.execute("SET ROLE $role") }
