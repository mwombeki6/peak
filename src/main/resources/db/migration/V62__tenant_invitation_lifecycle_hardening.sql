-- Prevent invitation acceptance from bypassing tenant-user lifecycle and role controls.

CREATE OR REPLACE FUNCTION accept_tenant_user_invitation(
    p_token_hash text,
    p_issuer text,
    p_subject text,
    p_email text,
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        WHERE tr.tenant_id = v_invitation.tenant_id
          AND tr.id = v_invitation.tenant_role_id
          AND tr.is_active = true
          AND tr.is_system = false
    ) THEN
        RAISE EXCEPTION 'Invitation tenant role is no longer assignable';
    END IF;
    ...
    IF v_user_id IS NOT NULL THEN
        RAISE EXCEPTION 'A tenant user already exists for this invitation email';
    END IF;
